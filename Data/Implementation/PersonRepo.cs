...
namespace MyTimeSheets.Data.Implementation
{
    public class PersonRepo : IPersonRepo

    {
        /// <summary>
        /// Список персонала с фамилией и именем, возрастом, местом работы и е-mail.
        /// </summary>
        private static List<Person> Sheets {get;set;} = new List<Person>(){ 
        ...
        {
            var deleteThisPerson=Sheets.FirstOrDefault( z => z.Id==ID);
              if (Sheets.Contains(deleteThisPerson)) 
                {
                    Sheets.Remove(deleteThisPerson);
                }
        }

        IEnumerable<Person> IRepository<Person>.GetAll()
        {
            var result = Sheets.ToArray();
            return result;
        }
        ...
        void IRepository<Person>.Update(Person item)
        {
             var updateDataPerson=Sheets.FirstOrDefault( y => y.Id==item.Id);
              if ( Sheets.Contains(updateDataPerson))
              { 
                int range =Sheets.IndexOf(updateDataPerson);
                Sheets.Remove(updateDataPerson);
                Sheets.Insert(range,item);
              }
        }
    }
}
